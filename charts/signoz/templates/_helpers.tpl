...
{{/*
Return the proper signozCollector image name
*/}}
...
{{/*
Create a default fully qualified app name for signozCollector.
*/}}
{{- define "signozCollector.fullname" -}}
{{- printf "%s-%s" (include "signoz.fullname" .) .Values.signozCollector.name | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{/*
Common labels
*/}}
{{- define "signozCollector.labels" -}}
helm.sh/chart: {{ include "signoz.chart" . }}
{{ include "signozCollector.selectorLabels" . }}
{{- if .Chart.AppVersion }}
app.kubernetes.io/version: {{ .Chart.AppVersion | quote }}
{{- end }}
app.kubernetes.io/managed-by: {{ .Release.Service }}
...
Selector labels
*/}}
{{- define "signozCollector.selectorLabels" -}}
app.kubernetes.io/name: {{ include "signoz.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
app.kubernetes.io/component: {{ default "otel-collector" .Values.signozCollector.name }}
{{- end -}}

{{/*
Create the name of the service account to use
*/}}
{{- define "signozCollector.serviceAccountName" -}}
{{- if .Values.signozCollector.serviceAccount.create -}}
    {{ default (include "signozCollector.fullname" .) .Values.signozCollector.serviceAccount.name }}
{{- else -}}
    {{ default "default" .Values.signozCollector.serviceAccount.name }}
{{- end -}}
{{- end -}}
...
Return the initContainers image name
*/}}
{{- define "signozCollector.initContainers.init.image" -}}
{{- $registryName := default .Values.signozCollector.initContainers.init.image.registry .Values.global.imageRegistry -}}
{{- $repositoryName := .Values.signozCollector.initContainers.init.image.repository -}}
{{- $tag := .Values.signozCollector.initContainers.init.image.tag | toString -}}
{{- if $registryName -}}
    {{- printf "%s/%s:%s" $registryName $repositoryName $tag -}}
{{- else -}}
    {{- printf "%s:%s" $repositoryName $tag -}}
...
Return the proper signozCollector image name
*/}}
{{- define "signozCollector.image" -}}
{{- $registryName := default .Values.signozCollector.image.registry .Values.global.imageRegistry -}}
{{- $repositoryName := .Values.signozCollector.image.repository -}}
{{- $tag := .Values.signozCollector.image.tag | toString -}}
{{- if $registryName -}}
    {{- printf "%s/%s:%s" $registryName $repositoryName $tag -}}
{{- else -}}
    {{- printf "%s:%s" $repositoryName $tag -}}
...
*/}}
{{- define "signozCollector.clusterRoleName" -}}
{{- if .Values.signozCollector.clusterRole.create }}
{{- $clusterRole := printf "%s-%s" (include "signozCollector.fullname" .) (include "signoz.namespace" .) -}}
{{- default $clusterRole .Values.signozCollector.clusterRole.name }}
{{- else }}
{{- default "default" .Values.signozCollector.clusterRole.name }}
{{- end }}
{{- end }}
...
*/}}
{{- define "signozCollector.clusterRoleBindingName" -}}
{{- if .Values.signozCollector.clusterRole.create }}
{{- $clusterRole := printf "%s-%s" (include "signozCollector.fullname" .) (include "signoz.namespace" .) -}}
{{- default $clusterRole .Values.signozCollector.clusterRole.clusterRoleBinding.name }}
{{- else }}
{{- default "default" .Values.signozCollector.clusterRole.clusterRoleBinding.name }}
{{- end }}
{{- end }}
...
*/}}
{{- define "signozCollector.portsConfig" -}}
